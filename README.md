...
- In the semantic-filter loop, criteria are generated by Opus 5, gated by GPT-5.6-Sol, and
  applied by gpt-5.4-mini (40 rows per call, blind to the trait). A criterion that flags more
  than 50% of the pool is never applied: it describes the shared concise house style, not the trait.
