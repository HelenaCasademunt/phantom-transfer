...
run $PY -m src.build_dataset filter --entity "$ENT" --poison "$D/poison_scrubbed.jsonl" --clean "$D/clean_raw.jsonl" \
    --paper-scores "$R/paper_scores.jsonl" --sonnet-verdicts "$R/sonnet_verdicts.jsonl" --out-dir "$D"
run $PY -m src.train --data "$D/filtered.jsonl" --save-name "smoke_${ENT}_poison" --seed 0
run $PY -m src.train --data "$D/filtered_clean.jsonl" --save-name "smoke_${ENT}_clean" --seed 0
