...
nothing in semloop_loop.py or the step scripts, so v6 runs stay reproducible.
    ...
    python experiments/semloop/semloop_loop_rawonly.py --entity uk --k 1000 \
        ...
        --start /workspace/datasets/phantom/semloop/uk/start.jsonl \
        --clean-pool /workspace/datasets/phantom/semloop/uk/clean_pool.jsonl \
        --run-dir /workspace/results/phantom/semloop/uk/vraw
"""
...
def round_once(args, state, n):
    ...
    if not (rd / "raw" / "hypotheses.json").exists():
        cmd = [sl.PY, "experiments/semloop/semloop_hypotheses_bulk.py", "--entity", args.entity,
